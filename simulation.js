...
function initLivingWorldMeshes(){
  ...
  const qgeo=new THREE.CircleGeometry(ENV_CELL_SIZE*.62,14);
  const wmat=new THREE.MeshStandardMaterial({
    color:0xffffff,vertexColors:true,transparent:true,opacity:.72,
    roughness:.12,metalness:.04,depthWrite:false
  });
  terrainWaterMesh=new THREE.InstancedMesh(qgeo,wmat,countCells);
...
function refreshLivingWorldVisuals(force=false){
  ...
  for(const c of livingWorldActive){
    ...
    const waterScale=water>.025
      ?clamp((c.pond?.92:.28)+Math.sqrt(water)*(c.pond?.28:.64),.25,1.13)
      :.001;
    matrix.compose(
      new THREE.Vector3(c.x+jitterX,terrainWaterSurfaceY(c),c.z+jitterZ),
      qq,new THREE.Vector3(waterScale,waterScale,1)
    );
    terrainWaterMesh.setMatrixAt(c.instance,matrix);
    waterColor.set(c.pond?0x287b99:0x3d8fa6);
    waterColor.lerp(new THREE.Color(0x78b6be),clamp(.20-water*.12,0,.18));
    terrainWaterMesh.setColorAt(c.instance,waterColor);
  ...
  if(terrainWaterMesh.instanceColor)terrainWaterMesh.instanceColor.needsUpdate=true;
...
function updateClimateLighting(){
  ...
  skyLight.intensity=.11+.95*Math.sqrt(light);
  skyLight.color.set(light>.18?0xb8dcf2:0x667187);
  skyLight.groundColor.set(light>.16?0x5f4b32:0x101915);
  ...
  renderer.toneMappingExposure=.80+.30*Math.sqrt(light);
}
