...
function randomBrain(ni=59,nh=22,no=18){
  return {
...
function randomGenome(){
  ...
  return {
    ...
    brain:randomBrain(59,hiddenSize,18)
  };
}
...
function mutateGenome(g){
  ...
  resizeBrain(ng.brain,c.hiddenSize,59,18);
...
function brainStep(a,inputs){
  ...
  if(!b.weights1[0] || inputs.length!==b.weights1[0].length){
    throw new Error(`Dimensión neural incompatible: entradas ${inputs.length}, cerebro ${b.weights1[0]?.length??0}`);
  }
  for(let j=0;j<nh;j++){
    let s=b.bias1[j];
