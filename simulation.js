...
const CONFIG={
  ...
  REPRO_MIN_ENERGY:66, REPRO_COST:24, REPRO_COOLDOWN:9.5,
  MIN_REPRO_AGE:22, JUVENILE_END:28, EMBRYO_MAX_AGE:110,
  EMBRYO_MAX:120, ARCHIVE_MAX:12, DEMO_WINDOW:100,
...
function updateAgent(a,dt){
  ...
  if(isFertile(a) && agents.length+embryos.length<CONFIG.MAX_AGENTS+CONFIG.EMBRYO_MAX){
    ...
    const agePeak=clamp(1-Math.abs((a.age-CONFIG.MIN_REPRO_AGE)-55)/95,.25,1);
    const density=agents.length/CONFIG.MAX_AGENTS;
    const ecologicalOpportunity=clamp(1.35-density*1.05,.22,1.35);
    const neuralMod=.58+.86*sigmoid(out[12]);
    const healthMod=clamp((a.health-50)/50,.2,1);
    const fertilityRate=.0125*energySurplus*agePeak*ecologicalOpportunity*neuralMod*healthMod;

    if(Math.random()<dt*fertilityRate){
...
function updateHUD(){
  ...
  document.getElementById('lineageState').textContent=extinct?'extinto':(agents.length?'vivo':'dormante');
...
function restoreWorld(s){
  ...
  worldAge=s.worldAge||0;originCount=s.originCount||1;extinct=!!s.extinct;extinctionAge=s.extinctionAge||0;
  ...
  const overlay=document.getElementById('extinction');if(overlay)overlay.style.display=extinct?'grid':'none';
}
function coarseOfflineAdvance(realSeconds,savedScale){
  ...
  if(agents.length===0&&embryos.length===0){extinct=true;extinctionAge=worldAge;paused=true;}
  else{extinct=false;paused=false;}
  return simAdvance;
}
...
function loop(now){
  ...
  if(!paused){
    ...
    for(let s=0;s<steps;s++){
      ...
      if(agents.length===0&&embryos.length===0&&!extinct){
        triggerExtinction();
        break;
      }
    }
