...
const MATERIAL_TYPES={
  fiber:{color:0xa4774c,mass:.55,hardness:.18,length:1.35,fertility:.10,energy:.5,bind:.92,flexibility:.92,edgePotential:.05,resonance:.18,durability:.42},
  stone:{color:0x9aa2aa,mass:1.45,hardness:.92,length:.55,fertility:0,energy:0,bind:.04,flexibility:.04,edgePotential:.64,resonance:.28,durability:.96},
  mineral:{color:0xd4c17a,mass:1.05,hardness:.72,length:.72,fertility:0,energy:0,bind:.08,flexibility:.08,edgePotential:.88,resonance:.82,durability:.82},
  biomass:{color:0x62a968,mass:.48,hardness:.10,length:.65,fertility:.85,energy:3.2,bind:.38,flexibility:.62,edgePotential:.02,resonance:.08,durability:.24}
};
...
function makeMaterial(type=null,pos=randomGroundPos(),custom=null){
  ...
  const fallbackComposite={color:0x92775d,mass:1,hardness:.5,length:1,fertility:0,energy:0,bind:.45,flexibility:.3,edgePotential:.3,resonance:.3,durability:.65,edge:.15,leverage:.5,complexity:1};
  const base=type==='composite'?(custom?.props||fallbackComposite):MATERIAL_TYPES[type];
  const p={...base};
...
function buildPerception(a){
  ...
  return {f,fd,n,nd,m,md,st,sd,sense,fVisible,nVisible,mVisible,sVisible,mem,heardPitch,heardAmp,heardToken,neighborSignals};
}
...
function updateAgent(a,dt){
  ...
  if(manipIntent>.28){
    if(!a.carrying&&p.m&&p.md<manipRadius){
      ...
      if(a.carrying){a.carrying.work=(a.carrying.work||0)+dt*Math.abs(manipIntent);a.carrying.manipCount=(a.carrying.manipCount||0)+dt;}
...
function snapshotWorld(){
  return {
    ...
    materials:materials.filter(m=>!m.carriedBy).map(m=>({type:m.type,pos:vecToArray(m.mesh.position),staticTime:m.staticTime,props:m.props,parts:m.parts,builders:m.builders,placedBy:m.placedBy,placedAt:m.placedAt,manipCount:m.manipCount,work:m.work})),
