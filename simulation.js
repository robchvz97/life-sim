...
let totalShapingEvents=0,totalProcedureCopies=0,totalFluidTransfers=0,totalMechanicalWork=0;
...
class SimpleOrbitControls{
  ...
  clampTarget(){
    ...
    if(r>maxR){
      const s=maxR/r;
      this.target.x*=s;this.target.z*=s;
    }
    this.target.y=0;
  }
  ...
  moveTarget(dt){
    ...
    const speed=this.keySpeed*boost*(.55+this.radius/45);
...
function updateThermodynamics(dt){
  ...
  for(const m of materials){
    ...
    if(inPond(pos)){target=Math.min(target,16);m.wetness=clamp(m.wetness+dt*.18,0,1);}else m.wetness=clamp(m.wetness-dt*.012,0,1);
...
function makeMaterial(type=null,pos=randomGroundPos(),custom=null){
  ...
  const m={
    ...
    mechanicalWork:custom?.mechanicalWork||0
  };
  materials.push(m);
...
function makeAgent(genome=randomGenome(),pos=randomGroundPos(),generation=0,lineage=null,opts={}){
  ...
  const a={
    ...
    markCooldown:rand(0,2),materialDiscoveries:0,
    procedureMemory:{history:[],library:new Map(),active:null,copied:0},
  ...
  if(!opts.originSeed)birthEvents.push(worldAge);
...
function fitnessScore(a){
  return (
    ...
    a.foodEaten*2.2 +
    a.energyGained*.10 +
    Math.max(0,a.health)*.04 +
    Math.max(0,a.energy)*.03 +
    Math.min(20,a.sequenceModel?.size||0)*.4
  );
}
...
function hatchEmbryo(e){
  ...
  const child=makeAgent(e.genome,p,e.generation,e.lineage,{parentIds:pids});
  if(!child)return false;
  child.parentId=pids[0]??null;child.parentIds=pids;
  child.energy=CONFIG.START_ENERGY*.92;
...
function setRigidMode(active){
  ...
  if(active){
    for(const a of agents)if(!a.physics)attachPhysicsRig(a);
  }else{
...
function mateCompatibility(a,b){
  ...
  return health*.27+energy*.25+rep*.18+novelty*.15*(c.noveltyMateBias||.4)-kin*.38*(c.kinBias||.5)+rand(-.03,.03);
}
...
function recordTransfer(giver,receiver,amount,type){
  ...
  g.given+=amount;g.help+=type==='food'?amount*.35:amount*.12;g.last=worldAge;g.trust+=.01;
  r.received+=amount;r.last=worldAge;r.trust+=.018;
...
function maybeTransferResources(a,p,dt){
  const b=p.n;if(!b||p.nd>1.25)return;
  const rec=a.socialLedger.get(b.id),debt=rec?clamp((rec.received-rec.given)/20,-1,1):0;
  const willingness=(a.genome.cognition?.generosity||.35)*.4+(a.genome.cognition?.reciprocityBias||.55)*debt*.25+
    kinshipScore(a,b)*(a.genome.cognition?.kinBias||.45)*.30+Math.max(0,reputationScore(a,b))*.18;
  if(a.energy>74&&b.energy<48&&Math.random()<1-Math.exp(-dt*.018*Math.max(0,willingness))){
    const amount=Math.min(8,a.energy-66,58-b.energy);
    if(amount>1){a.energy-=amount;b.energy+=amount*.92;recordTransfer(a,b,amount,'food');}
  }
  if(a.carrying&&!b.carrying&&Math.random()<1-Math.exp(-dt*.010*Math.max(0,willingness-.08))){
    const m=a.carrying;const rg=regionFor(a.mesh.position),rr=regionFor(b.mesh.position);
    dropMaterial(a,true);
    if(m&&!m.carriedBy&&m.mesh.position.distanceTo(b.mesh.position)<1.6){
      pickUpMaterial(b,m);recordTransfer(a,b,1,'material');if(rg!==rr)totalRegionalTrade++;
    }
  }
}
  ...
  for(const a of agents){
    ...
    while(q.length){
      ...
      for(const b of agents){
        ...
        if(x.mesh.position.distanceTo(b.mesh.position)<3.2&&(kinshipScore(x,b)>.15||reputationScore(x,b)>.18)){
          seen.add(b.id);q.push(b);
        }
      }
    }
    if(count>=3)groups++;
...
function makeWorldMark(a,pattern=null){
  if(worldMarks.length>=260)return null;
  const p=a.mesh.position.clone();p.x+=Math.sin(a.heading)*.48;p.z+=Math.cos(a.heading)*.48;p.y=0;
...
function updateWorldMarks(dt){
  for(let i=worldMarks.length-1;i>=0;i--){const m=worldMarks[i],age=worldAge-m.createdAt,persistence=900*(.6+m.strength*.8);if(age>persistence&&worldAge-m.lastUsed>180){markGroup.remove(m.group);m.group.traverse(o=>{if(o.geometry)o.geometry.dispose();if(o.material)o.material.dispose();});worldMarks.splice(i,1);}}
}
...
function languagePopulationMetrics(){
  const tokenUsers=new Map(),dialects=new Set();let score=0,nAgents=0;
  for(const a of agents){
    score+=a.language?.comprehension||0;nAgents++;
    const top=[...(a.language?.lexicon||new Map()).entries()].filter(([,r])=>r.n>1&&r.confidence>.24)
      .sort((x,y)=>(y[1].confidence*y[1].n)-(x[1].confidence*x[1].n)).slice(0,4).map(([t])=>t);
    for(const t of top)tokenUsers.set(t,(tokenUsers.get(t)||0)+1);
    if(top.length>=2)dialects.add(top.slice().sort().join('|'));
  }
  return {shared:[...tokenUsers.values()].filter(n=>n>=2).length,dialects:dialects.size,score:nAgents?score/nAgents:0};
}
...
function procedureKey(seq){return seq.join('>');}
function updateProcedureMemory(a,reward){
  const pm=a.procedureMemory;
  pm.history.push(a.decision.mode);
  if(pm.history.length>5)pm.history.shift();
  if(pm.history.length<3)return;
  const seq=pm.history.slice(-3),key=procedureKey(seq);
  let r=pm.library.get(key);
  if(!r){r={seq:[...seq],q:0,n:0,source:a.id};pm.library.set(key,r);}
  r.n++;const alpha=clamp(.15/Math.sqrt(r.n),.025,.15);r.q+=alpha*(reward-r.q);
  if(pm.library.size>42){
    pm.library=new Map([...pm.library.entries()].sort((x,y)=>(y[1].q+y[1].n*.003)-(x[1].q+x[1].n*.003)).slice(0,34));
  }
}
function procedureBias(a,mode){
  const h=a.procedureMemory.history;if(h.length<2)return 0;
  const suffix=h.slice(-2);let best=0;
  for(const r of a.procedureMemory.library.values()){
    if(r.seq[0]===suffix[0]&&r.seq[1]===suffix[1]&&r.seq[2]===mode)best=Math.max(best,r.q);
  }
  return best;
}
function imitateProcedure(a,n,dt){
  if(!n?.procedureMemory?.library?.size||n.lastReward<.04)return;
  const rate=.035*(a.genome.cognition?.imitation||.5)*(a.genome.cognition?.cultureBias||.5);
  if(Math.random()>1-Math.exp(-dt*rate))return;
  const best=[...n.procedureMemory.library.entries()].sort((x,y)=>y[1].q-x[1].q)[0];
  if(!best||best[1].q<=.02)return;
  const [key,r]=best,cur=a.procedureMemory.library.get(key);
  if(!cur||r.q*.55>cur.q){
    a.procedureMemory.library.set(key,{seq:[...r.seq],q:r.q*.20,n:.2,source:n.id});
    a.procedureMemory.copied++;totalProcedureCopies++;
    if(totalProcedureCopies===1)recordDiscovery('procedure:first','🧭','Primer procedimiento copiado','Un organismo imitó una secuencia de decisiones útil de otro.');
  }
}
function exportProcedures(a){
  return [...a.procedureMemory.library.entries()].filter(([,r])=>r.q>.02)
    .sort((x,y)=>y[1].q-x[1].q).slice(0,4)
    .map(([k,r])=>[k,{seq:[...r.seq],q:r.q*.12,n:.1,source:a.id}]);
}
function importProcedures(a,seed,strength=.7){
  for(const [k,r] of seed||[])a.procedureMemory.library.set(k,{seq:[...r.seq],q:r.q*strength,n:(r.n||.1)*strength,source:r.source});
}
function procedureCount(){return agents.reduce((s,a)=>s+(a.procedureMemory?.library?.size||0),0);}
...
function chooseDecision(a,per,out){
  ...
  for(const c of candidates){
    ...
    score+=procedureBias(a,c.mode)*.24*(a.genome.cognition?.cultureBias||.5);
    if(per.nVisible&&per.n){const kin=kinshipScore(a,per.n),rep=reputationScore(a,per.n);if(c.mode===6)score+=(kin*(a.genome.cognition?.kinBias||.4)+Math.max(0,rep)*(a.genome.cognition?.reciprocityBias||.5))*.06;if(c.mode===7)score+=(Math.max(0,-rep)*(a.genome.cognition?.territoriality||.3)-kin*.08)*.05;}
...
function maybeCreateStructure(dt){
  ...
  const candidates=materials.filter(m=>!m.carriedBy&&m.staticTime>1.25&&m.placedBy!=null);
...
function applyFluidAndMechanics(a,p,manipIntent,dt){
  ...
  if(inPond(a.mesh.position)&&manipIntent>.35){
    const before=held.waterAmount||0;
    held.waterAmount=clamp(before+dt*.20*cap,0,cap);
    held.waterTemp+=(16-held.waterTemp)*clamp(dt*.3,0,.4);
    if(before<.015&&held.waterAmount>.03){
      recordDiscovery('container:water:first','💧','Primer transporte de agua','Un objeto compuesto retuvo agua fuera del estanque.',a.mesh.position);
    }
  }
  if(p.m&&p.m!==held&&p.md<1.3&&held.waterAmount>.025&&manipIntent>.62){
    const amount=Math.min(held.waterAmount,dt*.10),old=p.m.temperature;
    p.m.temperature+=(held.waterTemp-p.m.temperature)*clamp(amount*.9,0,.32);
    held.waterAmount-=amount;totalFluidTransfers+=amount;
    if(old-p.m.temperature>3){
...
function updateAgent(a,dt){
  ...
  if(a.decision.timer<=0){
    ...
    updateProcedureMemory(a,clamp(a.rewardBuffer,-1,1));
  ...
  if(!rigidModeActive||!a.physics){
    ...
    a.motor.distance+=moved;
    if(a.decision.target&&moved>0)a.rewardBuffer+=moved*.0035*g.motorPlasticity;
  }else{
    // Reward actual physical locomotion, not desired locomotion.
    const physicalSpeed=physicsVelocity(a);
    if(a.decision.target&&physicalSpeed>.015){
      a.rewardBuffer+=dt*physicalSpeed*.003*g.motorPlasticity;
    }
  }
  ...
  a.energy-=dt*(basal+moveCost*youthCostFactor+carryCost+motorCost+(inPond(a.mesh.position)?CONFIG.WATER_DRAIN:0))*shelter;
  ...
  if(manipIntent>.28){
    if(!a.carrying&&p.m&&p.md<manipRadius){
      ...
      const rolling=1+(p.m.props.rotationEfficiency||0)*1.35;
      const pushStrength=g.grip*toolBonus*rolling/(p.m.props.mass+.2);
      const dir=new THREE.Vector3(Math.sin(a.heading),0,Math.cos(a.heading)),before=p.m.mesh.position.clone();
      p.m.mesh.position.addScaledVector(dir,dt*.7*pushStrength);
      const moved=p.m.mesh.position.distanceTo(before);
      shapeAndMoveMaterial(a,p.m,tool,manipIntent,dt,moved);
      p.m.placedBy=a.id;p.m.placedAt=worldAge;p.m.manipCount=(p.m.manipCount||0)+dt;
      if(a.carrying){
        a.carrying.work=(a.carrying.work||0)+dt*Math.abs(manipIntent);a.carrying.manipCount=(a.carrying.manipCount||0)+dt;
        const tool=a.carrying.props;
        const contactWork=dt*Math.abs(manipIntent)*(tool.hardness||0)*(.5+(tool.edge||tool.edgePotential||0));
        p.m.abrasion=(p.m.abrasion||0)+contactWork/Math.max(.08,p.m.props.abrasionResistance||.5);
        p.m.fracture=(p.m.fracture||0)+contactWork*.32/Math.max(.08,p.m.props.fractureResistance||.5);
        if(p.m.abrasion>.7){p.m.abrasion=0;p.m.props.edgePotential=clamp((p.m.props.edgePotential||0)+.04*(tool.hardness||0),0,1.4);p.m.props.hardness=clamp((p.m.props.hardness||0)+.012,0,1.35);a.materialDiscoveries++;}
        if(p.m.fracture>1.25){p.m.fracture=.15;p.m.props.mass=Math.max(.12,p.m.props.mass*.86);p.m.props.length=Math.max(.22,p.m.props.length*.88);p.m.props.edgePotential=clamp((p.m.props.edgePotential||0)+.10,0,1.4);p.m.props.durability=Math.max(.08,p.m.props.durability*.90);a.rewardBuffer+=.008*t.curiosity;}
      }
  ...
  if(a.carrying&&manipIntent>.70&&a.markCooldown<=0){
    const tool=a.carrying.props,markAbility=(tool.hardness||0)*.45+(tool.edge||tool.edgePotential||0)*.55;
    if(markAbility>.42&&Math.random()<1-Math.exp(-dt*.16*markAbility)){
      const scene=sceneCode(a,p),known=a.externalMemory.production.get(scene),mark=makeWorldMark(a,known?.q>.015?known.pattern:null);
      if(mark){a.externalMemory.lastProduced=mark.pattern;a.externalMemory.lastScene=scene;a.markCooldown=rand(1.8,4.5);}
    }
  }
  ...
  if(isFertile(a) && agents.length+embryos.length<CONFIG.MAX_AGENTS+CONFIG.EMBRYO_MAX){
    ...
    if(mate){
      ...
      const density=agents.length/CONFIG.MAX_AGENTS;
      const ecology=clamp(1.45-density*1.15,.22,1.45);
      const lowBoost=agents.length<12?3.0:agents.length<30?1.9:agents.length<60?1.22:1;
      const compatibility=clamp(mateCompatibility(a,mate)+.20,.15,1.35);
      const fertilityRate=.038*(.22+.39*ea+.39*eb)*ecology*lowBoost*compatibility;

      if(Math.random()<1-Math.exp(-dt*fertilityRate)){
        const cost=CONFIG.REPRO_COST*.62;
        a.energy=Math.max(25,a.energy-cost);mate.energy=Math.max(25,mate.energy-cost*.82);
        ...
        if(Math.random()<clamp(.16+(agents.length<24?.12:0),.12,.36)){
          ...
          if(e){e.parentIds=[a.id,mate.id];e.languageSeed=[...exportLanguageSeed(a),...exportLanguageSeed(mate)].slice(0,6);e.motorSeed=[...exportMotorCulture(a),...exportMotorCulture(mate)].slice(0,4);e.procedureSeed=[...exportProcedures(a),...exportProcedures(mate)].slice(0,5);}
        }else{
          const child=makeAgent(childGenome,pos,generation,lineage,{parentIds:[a.id,mate.id]});
          if(child){
            child.energy=CONFIG.START_ENERGY*.95;child.phenotype.growth=.22+rand(0,.08);
        ...
        totalMateEvents++;a.rewardBuffer+=.035;mate.rewardBuffer+=.025;
      }
    }
  }
...
function updateHUD(){
  ...
  document.getElementById('socialGroups').textContent=socialGroupCount();
  ...
  if(physicsEl)physicsEl.textContent=!physicsEnabled?'respaldo':(rigidModeActive?'Rapier · rígida':'turbo híbrido');
...
function snapshotAgent(a){
  return {
    ...
    motor:a.motor,pos:vecToArray(a.mesh.position),heading:a.heading,targetHeading:a.targetHeading,
    ...
    materialDiscoveries:a.materialDiscoveries||0,
    ...
    carryingData:a.carrying?{type:a.carrying.type,props:a.carrying.props,waterAmount:a.carrying.waterAmount,waterTemp:a.carrying.waterTemp,softness:a.carrying.softness,shaping:a.carrying.shaping,rotationTurns:a.carrying.rotationTurns,mechanicalWork:a.carrying.mechanicalWork,temperature:a.carrying.temperature,wetness:a.carrying.wetness,thermalState:a.carrying.thermalState,transformedLevel:a.carrying.transformedLevel}:null
  };
}
...
function snapshotWorld(){
  return {
    ...
    worldAge,births,deaths,nextId,nextMaterialId,nextStructureId,totalMentalSims,totalImitations,
    ...
    totalShapingEvents,totalProcedureCopies,totalFluidTransfers,totalMechanicalWork,
...
function restoreAgentSnapshot(s){
  ...
  a.materialDiscoveries=s.materialDiscoveries||0;
  ...
  if(s.procedureMemory){a.procedureMemory={...a.procedureMemory,...s.procedureMemory,library:mapFromEntries(s.procedureMemory.library),history:s.procedureMemory.history||[],active:null};}
...
function restoreWorld(s){
  ...
  totalFluidTransfers=s.totalFluidTransfers||0;totalMechanicalWork=s.totalMechanicalWork||0;
  ...
  births=s.births||agents.length;deaths=s.deaths||0;totalMentalSims=s.totalMentalSims||0;totalImitations=s.totalImitations||0;
...
function coarseOfflineAdvance(realSeconds,savedScale){
  ...
  for(let c=0;c<cycles;c++){
    ...
    for(const a of [...agents]){
      ...
      const maxAge=(CONFIG.MAX_AGE/a.genome.body.agingRate)*(.75+a.genome.body.size*.35);
...
function seedActiveFromBank(){
  ...
  applyMorphologyVisual(a);if(a.physics)rebuildPhysicsRig(a);biosphereRescues++;return a;
}
function emergencyDemographicRecovery(reason='población crítica'){
  if(worldAge-lastRescueAt<8)return 0;
  const before=agents.length,target=CONFIG.CULTURE_TARGET_POP;
  while(agents.length<target)if(!seedActiveFromBank())break;
  for(let i=embryos.length;i<Math.min(8,CONFIG.EMBRYO_MAX);i++)addDormantSeedFromBank();
  lastRescueAt=worldAge;demographicRescueActive=true;
  const added=agents.length-before;
  if(added>0)recordDiscovery(`rescue:${Math.floor(worldAge/8)}`,'🌱','Recuperación demográfica',`${reason}: se reactivó el banco evolutivo con ${added} individuos para conservar la biosfera global.`);
  return added;
}
function stabilizeLoadedPopulation(){
  const noFertile=fertilePopulation()===0;
  if(agents.length<12 || (agents.length<CONFIG.CULTURE_MIN_POP&&noFertile&&averageAge()>45)){
    emergencyDemographicRecovery('mundo guardado sin población reproductiva suficiente');
  }
}
...
function demographicRiskLabel(){
  ...
  if(agents.length<8)return 'crítico';
  if(agents.length<18||demo.balance<-8)return 'alto';
  if(agents.length<35||demo.balance<0)return 'medio';
  return 'estable';
}
...
function maintainBiosphereContinuity(dt){
  ...
  if(agents.length<12 || (agents.length<CONFIG.CULTURE_MIN_POP&&fertile===0&&averageAge()>48)){
    const urgency=agents.length<8?.22:.075;
    if(Math.random()<1-Math.exp(-dt*urgency))emergencyDemographicRecovery('población crítica sin reemplazo generacional');
  }else if(agents.length<CONFIG.CULTURE_MIN_POP&&embryos.length<8){
    const severity=1-agents.length/CONFIG.CULTURE_MIN_POP;
    const rate=.018+.075*severity;
    if(Math.random()<1-Math.exp(-dt*rate))addDormantSeedFromBank();
  }else if(agents.length>=CONFIG.CULTURE_MIN_POP){
...
function seed(){
  ...
  totalShapingEvents=0;totalProcedureCopies=0;totalFluidTransfers=0;totalMechanicalWork=0;
  while(worldMarks.length){const wm=worldMarks.pop();markGroup.remove(wm.group);wm.group?.traverse(o=>{if(o.geometry)o.geometry.dispose();if(o.material)o.material.dispose();});}
