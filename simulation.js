...
function maybeTransferResources(a,p,dt){
  const b=p.n;if(!b||p.nd>1.35)return;
  const rec=a.socialLedger.get(b.id);
  const balance=rec?(rec.received-rec.given):0;
  const owed=clamp(balance/9,0,1);
  const debt=clamp(balance/20,-1,1);
  const reciprocity=a.genome.cognition?.reciprocityBias||.55;
  const willingness=(a.genome.cognition?.generosity||.35)*.36+reciprocity*debt*.28+
    kinshipScore(a,b)*(a.genome.cognition?.kinBias||.45)*.28+Math.max(0,reputationScore(a,b))*.20;

  const needGift=a.energy>67&&b.energy<60;
  const returnGift=owed>.16&&a.energy>60&&a.energy>b.energy+4;
  const giftRate=(needGift?.034:0)+(returnGift?.080*reciprocity*owed:0);

  if(giftRate>0&&Math.random()<1-Math.exp(-dt*giftRate*Math.max(.18,willingness+.22))){
    const desired=returnGift?3+4*owed:Math.max(2,62-b.energy);
    const amount=Math.min(7,a.energy-57,desired);
    if(amount>1){
      a.energy-=amount;b.energy=clamp(b.energy+amount*.92,0,CONFIG.MAX_ENERGY);
      recordTransfer(a,b,amount,'food');
    }
  }

  const materialReturn=owed>.18?1+.9*owed:1;
  if(a.carrying&&!b.carrying&&Math.random()<1-Math.exp(-dt*.017*Math.max(0,willingness)*materialReturn)){
    const m=a.carrying;
    dropMaterial(a,true);
    if(m&&!m.carriedBy&&m.mesh.position.distanceTo(b.mesh.position)<1.7){
...
function maybeLeaveSurfaceTrace(a,p,manipIntent,dt,speed){
  if(!a.carrying||a.markCooldown>0)return;
  const tool=a.carrying.props;
  const ability=(tool.hardness||0)*.42+(tool.edge||tool.edgePotential||0)*.58;
  if(ability<.18)return;
  const movement=clamp(speed/Math.max(.35,a.genome.body.speed||1),0,1.5);
  const pressure=clamp((a.motor?.effort||0)*.45+Math.abs(manipIntent)*.55,0,1.5);
  const scrape=ability*(.22+.45*movement+.55*pressure);
  if(scrape<.16)return;

  if(Math.random()<1-Math.exp(-dt*.075*scrape)){
...
function procedureCulturalScore(raw){
  ...
  const confidence=clamp(r.n/(r.n+5),0,1);
  ...
  return Math.max(0,r.q)*confidence*(.42+.58*success)+usefulChange*.045+Math.log1p(r.transmissions||0)*.004+recency*.003;
}
function stableProcedure(raw){
  const r=normalizeProcedureRecord(raw);
  return !!(r?.steps?.length&&r.n>=4&&r.successes>=2&&r.q>.018&&procedureCulturalScore(r)>.016);
}
function updateProcedureMemory(a,p,reward){
  ...
  if(pm.pending){
    ...
    if(pm.history.length>=3){
      ...
      if(reward>.025){r.successes++;if(changed)r.beneficialChanges++;}
      else if(reward<-.015)r.failures++;
