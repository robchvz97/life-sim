...
let terrainSurfaceMesh=null,terrainWaterMesh=null;
let livingWorldAccumulator=0,livingVisualAccumulator=0;

const DAY_SKY=new THREE.Color(0x86b9d6);
const DUSK_SKY=new THREE.Color(0x684b58);
const NIGHT_SKY=new THREE.Color(0x08111b);
const DAY_FOG=new THREE.Color(0x708f91);
...
function baseClimateTemperature(pos){
  ...
  return 12+14*light*sunFactor+seasonTemperatureOffset()-elevation*2.2-wet*1.6;
}
function initLivingWorldMeshes(){
  if(terrainSurfaceMesh){
    terrainGroup.remove(terrainSurfaceMesh);
    terrainSurfaceMesh.geometry.dispose();terrainSurfaceMesh.material.dispose();
    terrainSurfaceMesh=null;
  }
  if(terrainWaterMesh){
    terrainGroup.remove(terrainWaterMesh);
    terrainWaterMesh.geometry.dispose();terrainWaterMesh.material.dispose();
    terrainWaterMesh=null;
  }
  const count=Math.max(1,livingWorldActive.length);
  const geo=new THREE.PlaneGeometry(ENV_CELL_SIZE*.985,ENV_CELL_SIZE*.985);
  const mat=new THREE.MeshStandardMaterial({vertexColors:true,roughness:.93,metalness:0});
  terrainSurfaceMesh=new THREE.InstancedMesh(geo,mat,count);
  terrainSurfaceMesh.receiveShadow=true;
  terrainSurfaceMesh.frustumCulled=false;
  terrainGroup.add(terrainSurfaceMesh);

  const wgeo=new THREE.PlaneGeometry(ENV_CELL_SIZE*.91,ENV_CELL_SIZE*.91);
  const wmat=new THREE.MeshStandardMaterial({
    color:0x2b7894,transparent:true,opacity:.58,roughness:.20,metalness:.05,
    depthWrite:false
  });
  terrainWaterMesh=new THREE.InstancedMesh(wgeo,wmat,count);
  terrainWaterMesh.frustumCulled=false;
  ...
  for(let iz=0;iz<ENV_GRID;iz++){
    for(let ix=0;ix<ENV_GRID;ix++){
      ...
      const elevation=clamp(
        Math.sin(x*.115)*.30+Math.cos(z*.097)*.25+
        Math.sin((x+z)*.061)*.22+(n1-.5)*.36,-1,1
      );
...
function refreshLivingWorldVisuals(force=false){
  if(!terrainSurfaceMesh||!terrainWaterMesh)return;
  const q=new THREE.Quaternion().setFromEuler(new THREE.Euler(-Math.PI/2,0,0));
  const matrix=new THREE.Matrix4(),waterMatrix=new THREE.Matrix4();
  const soilDry=new THREE.Color(0x80633e),soilWet=new THREE.Color(0x4b4934);
  const grass=new THREE.Color(0x447b3c),lush=new THREE.Color(0x56a846);
  const pathColor=new THREE.Color(0x766047);
  const color=new THREE.Color();
  for(const c of livingWorldActive){
    const wet=clamp(c.moisture,0,1),bio=clamp(c.biomass,0,1),compact=clamp(c.compaction,0,1);
    color.copy(soilDry).lerp(soilWet,wet*.78);
    color.lerp(grass,clamp(bio*.82,0,.78));
    if(bio>.55)color.lerp(lush,clamp((bio-.55)*.75,0,.28));
    if(compact>.12)color.lerp(pathColor,clamp(compact*.48,0,.42));
    const variation=.88+c.textureNoise*.20;
    color.multiplyScalar(variation);
    terrainSurfaceMesh.setColorAt(c.instance,color);

    const y=.010+c.elevation*.006;
    matrix.compose(
      new THREE.Vector3(c.x,y,c.z),q,new THREE.Vector3(1,1,1)
    );
    terrainSurfaceMesh.setMatrixAt(c.instance,matrix);

    const water=clamp(c.surfaceWater,0,1.2);
    const ws=water>.035?clamp(.58+water*.34,.58,.98):.001;
    waterMatrix.compose(
      new THREE.Vector3(c.x,.026+c.elevation*.006+water*.010,c.z),
      q,new THREE.Vector3(ws,ws,1)
    );
    terrainWaterMesh.setMatrixAt(c.instance,waterMatrix);
  }
  terrainSurfaceMesh.instanceMatrix.needsUpdate=true;
  terrainWaterMesh.instanceMatrix.needsUpdate=true;
  if(terrainSurfaceMesh.instanceColor)terrainSurfaceMesh.instanceColor.needsUpdate=true;
}
function lowerNeighbor(c){
  let best=null,bestH=c.elevation+c.surfaceWater*.10;
  ...
  for(const [dx,dz] of dirs){
    ...
    const h=n.elevation+n.surfaceWater*.10;
    if(h<bestH-.012){bestH=h;best=n;}
  }
  return best;
}
function updateLivingWorld(dt){
  ...
  for(const c of livingWorldActive){
    ...
    if(c.surfaceWater>.055){
      ...
      if(n){
        const slope=clamp((c.elevation-n.elevation)*.65+c.surfaceWater*.16,0,.45);
    ...
    const stress=(c.moisture<.16?.0034:0)+(c.moisture>1.02?.0030:0)+(c.surfaceWater>.50?.0045:0);
    c.biomass=clamp(c.biomass+growth-stress*h,0,1.12);
  ...
  if(livingVisualAccumulator>2.4){
    livingVisualAccumulator=0;refreshLivingWorldVisuals();
  }
}
function livingWorldMetrics(){
  if(!livingWorldActive.length)return {moisture:0,vegetation:0,water:0,compacted:0};
  let m=0,v=0,w=0,c=0;
  for(const x of livingWorldActive){
    m+=x.moisture;w+=x.surfaceWater;
    if(x.biomass>.28)v++;
    if(x.compaction>.24)c++;
  }
  const n=livingWorldActive.length;
  return {moisture:m/n,vegetation:v/n,water:w/n,compacted:c/n};
}
...
function terrainMovementFactor(pos){
  ...
  return clamp(1-mud*.22-shallow*.34+pathBonus,.48,1.08);
}
...
function updateClimateLighting(){
  ...
  sun.position.set(Math.cos(ang)*28,4+light*27,Math.sin(ang)*28);
  sun.intensity=(.07+1.35*light)*sunFactor;
  skyLight.intensity=.18+.78*light;
  skyLight.color.set(light>.18?0xaad7ff:0x506079);
  const dusk=Math.max(0,1-Math.abs(light-.18)/.18);
  const sky=scene.background;
  sky.copy(NIGHT_SKY).lerp(DAY_SKY,light);
  if(dusk>0)sky.lerp(DUSK_SKY,dusk*.26);
  scene.fog.color.copy(NIGHT_FOG).lerp(DAY_FOG,light*.72);
}
...
function chooseLocalWaypoint(a,goal){
  ...
  for(const off of offsets){
    ...
    const score=progress*1.05-obstacle*.82-turn*.08+escapeBias;
...
function collectiveMaterialEffort(a,m,manipIntent){
  ...
  let vx=Math.sin(a.heading)*a.genome.body.grip*ownMorph.strength;
  let vz=Math.cos(a.heading)*a.genome.body.grip*ownMorph.strength;
  ...
  for(const o of agents){
    ...
    const force=o.genome.body.grip*om.strength*clamp(Math.abs(o.currentManipIntent||0),.24,1);
...
function materialInteractionStep(a,m,manipIntent,h,t){
  ...
  const ownCapacity=Math.max(.10,g.grip*morphologyMetrics(a).strength);
...
function updateAgent(a,dt){
  ...
  if(!rigidModeActive||!a.physics){
    ...
    speed*=alignment;
    speed*=terrainMovementFactor(a.mesh.position);
  ...
  const substrateEffort=1/terrainMovementFactor(a.mesh.position);
