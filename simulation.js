...
const MATERIAL_TYPES={
  fiber:{color:0xa4774c,mass:.55,hardness:.18,length:1.35,fertility:.10,energy:.5,bind:.92,flexibility:.92,edgePotential:.05,resonance:.18,durability:.42,heatCapacity:.72,conductivity:.18,flammability:.96,ignitionTemp:215,thermalTransform:360,fractureResistance:.22,abrasionResistance:.18},
  stone:{color:0x9aa2aa,mass:1.45,hardness:.92,length:.55,fertility:0,energy:0,bind:.04,flexibility:.04,edgePotential:.64,resonance:.28,durability:.96,heatCapacity:.86,conductivity:.62,flammability:0,ignitionTemp:9999,thermalTransform:680,fractureResistance:.82,abrasionResistance:.91},
  mineral:{color:0xd4c17a,mass:1.05,hardness:.72,length:.72,fertility:0,energy:0,bind:.08,flexibility:.08,edgePotential:.88,resonance:.82,durability:.82,heatCapacity:.58,conductivity:.84,flammability:0,ignitionTemp:9999,thermalTransform:520,fractureResistance:.65,abrasionResistance:.70},
  biomass:{color:0x62a968,mass:.48,hardness:.10,length:.65,fertility:.85,energy:3.2,bind:.38,flexibility:.62,edgePotential:.02,resonance:.08,durability:.24,heatCapacity:.78,conductivity:.15,flammability:.86,ignitionTemp:175,thermalTransform:310,fractureResistance:.18,abrasionResistance:.12}
};
...
function createHeatSources(){
  ...
  for(const d of data){
    ...
    const glow=new THREE.Mesh(new THREE.RingGeometry(.45,1.15,24),new THREE.MeshBasicMaterial({color:0xff5a22,transparent:true,opacity:.15,side:THREE.DoubleSide}));
...
function updateMaterialAppearance(m){
  ...
  if(m.burning){mat.emissive?.setHex(0xff3a08);mat.emissiveIntensity=1.2;}
...
function updateThermodynamics(dt){
  ...
  for(const m of materials){
    ...
    if(!m.burning&&(m.props.flammability||0)>.05&&m.temperature>(m.props.ignitionTemp||9999)&&m.wetness<.62){
      ...
      if(Math.random()<1-Math.exp(-dt*chance*.24)){m.burning=true;m.burnTime=0;totalIgnitions++;updateMaterialAppearance(m);}
    }
    if(m.burning){
...
function makeCompositeArtifact(chosen,center){
  ...
  return makeMaterial('composite',center,{props,parts,builders,placedBy:builders[0]??null,placedAt:worldAge,manipCount,work,staticTime:0});
}
...
function hatchEmbryo(e){
  ...
  const child=makeAgent(e.genome,p,e.generation,e.lineage);
  if(!child)return false;
  child.parentId=e.parentId;
...
function createMarkMesh(pattern,pos,angle=0,strength=1){
  ...
  for(let i=0;i<count;i++){
    const bar=new THREE.Mesh(new THREE.BoxGeometry(.42+.08*h,.018,.045),new THREE.MeshStandardMaterial({color:0x2a2622,roughness:1}));
...
function makeWorldMark(a,pattern=null){
  ...
  m.group=createMarkMesh(m.pattern,m.pos,m.angle,m.strength);worldMarks.push(m);return m;
}
...
function imitateMotorTechnique(a,n,dt){
  ...
  if(!cur||quality>cur.q){a.motorCulture.library.set(key,{vec:avg,q:quality,n:.25,source:n.id});a.motorCulture.copied++;totalMotorImitations++;}
...
function tryAttachMaterialToStructure(a,m){
  ...
  if(a&&!best.builders.includes(a.id))best.builders.push(a.id);
...
function updateAgent(a,dt){
  ...
  if(manipIntent>.28){
    if(!a.carrying&&p.m&&p.md<manipRadius){
      ...
      const pushStrength=g.grip*toolBonus/(p.m.props.mass+.2);
  ...
  // Reproduction is physiological first and cognitively modulated second.
  if(isFertile(a) && agents.length+embryos.length<CONFIG.MAX_AGENTS+CONFIG.EMBRYO_MAX){
    const energySurplus=clamp((a.energy-CONFIG.REPRO_MIN_ENERGY)/(CONFIG.MAX_ENERGY-CONFIG.REPRO_MIN_ENERGY),0,1);
    const usableEnergy=.20+.80*energySurplus;
    const agePeak=clamp(1-Math.abs((a.age-CONFIG.MIN_REPRO_AGE)-55)/95,.30,1);
    const density=agents.length/CONFIG.MAX_AGENTS;
    const ecologicalOpportunity=clamp(1.45-density*1.15,.22,1.45);
    const neuralMod=.62+.78*sigmoid(out[12]);
    const healthMod=clamp((a.health-50)/50,.25,1);
    const lowPopulationBoost=agents.length<12?2.8:agents.length<30?1.75:agents.length<60?1.18:1;
    const fertilityRate=.042*usableEnergy*agePeak*ecologicalOpportunity*neuralMod*healthMod*lowPopulationBoost;

    if(Math.random()<1-Math.exp(-dt*fertilityRate)){
      a.energy-=CONFIG.REPRO_COST;
      a.reproCooldown=CONFIG.REPRO_COOLDOWN*rand(.85,1.20);

      const childGenome=mutateGenome(a.genome);
      const childPos=a.mesh.position.clone();
      childPos.x+=rand(-.7,.7);childPos.z+=rand(-.7,.7);

      const dormancyChance=clamp(.22+(1-ecologicalOpportunity)*.12+(agents.length<24?.15:0),.12,.48);
      if(Math.random()<dormancyChance){
        createEmbryo(a,childGenome,childPos,a.generation+1,a.lineage);
      }else{
        const child=makeAgent(childGenome,childPos,a.generation+1,a.lineage);
        if(child){
          child.energy=CONFIG.START_ENERGY*.92;
          child.parentId=a.id;
          ...
          child.phenotype.condition=.94+rand(-.06,.08);
          applyMorphologyVisual(child);
          ...
          for(const [key,rec] of top){
            if(rec.q>0)child.sequenceModel.set(key,{q:rec.q*.10*cultureBias,n:.1});
          }
          importLanguageSeed(child,exportLanguageSeed(a),.70);
          importMotorCulture(child,exportMotorCulture(a),.62);
          const markSeed=[...a.externalMemory.lexicon.entries()].filter(([,r])=>r.success>0).slice(0,3);
          for(const [pat,r] of markSeed)child.externalMemory.lexicon.set(pat,{...r,contexts:{},modeQ:r.modeQ.map(v=>v*.06),n:Math.min(.4,r.n*.06),success:0});
        }
      }
      a.children++;
      a.rewardBuffer+=.04;
...
function updateFood(dt){
  ...
  if(food.length<CONFIG.FOOD_MAX&&Math.random()<dt*CONFIG.FOOD_RESPAWN*sunFactor*ecologicalRecovery)makeFood();
  for(const m of materials){
...
function updateMaterials(dt){
  ...
  updateThermodynamics(dt);updateWorldMarks(dt);
...
function restoreAgentSnapshot(s){
  ...
  if(a.physics)rebuildPhysicsRig(a);
  if(s.carryingType){
    const m=makeMaterial(s.carryingType,a.mesh.position.clone());
...
function restoreWorld(s){
  ...
  genomeArchive.push(...(s.genomeArchive||[]));birthEvents.push(...(s.birthEvents||[]));deathEvents.push(...(s.deathEvents||[]));
...
async function initPersistenceAndWorld(){
  ...
  try{
    ...
    if(saved?.schema===1){
      ...
      if(lastOfflineSeconds>0)await saveWorld();
    }else{
...
function maintainBiosphereContinuity(dt){
  ...
  // When population becomes dangerously small, a slow trickle from the dormant
  // seed bank reduces accidental total loss without replacing normal reproduction.
  if(agents.length<CONFIG.BIOSPHERE_LOW_POP && embryos.length<6){
    const severity=1-agents.length/CONFIG.BIOSPHERE_LOW_POP;
    const rate=.010+.055*severity;
