...
const CONFIG={
  ...
  MAX_AGENTS:280, FOOD_MAX:650, MATERIAL_MAX:275, MAX_STRUCTURES:100,
...
function snapshotStructure(s){
  return {
    id:s.id,pos:vecToArray(s.group.position),parts:s.parts,totalMass:s.totalMass,
    hardness:s.hardness,fertility:s.fertility,length:s.length,age:s.age,
...
function snapshotWorld(){
  return {
    ...
    materials:materials.filter(m=>!m.carriedBy).map(m=>({type:m.type,pos:vecToArray(m.mesh.position),staticTime:m.staticTime})),
...
function restoreStructureSnapshot(ss){
  ...
  const parts=ss.parts||[];
  for(let i=0;i<parts.length;i++){
    const temp=makeMaterial(parts[i],group.position.clone());
    if(!temp)continue;
    const mi=materials.indexOf(temp);if(mi>=0)materials.splice(mi,1);
    materialGroup.remove(temp.mesh);
    temp.mesh.position.copy(arrToVec(ss.childPositions?.[i]||[0,.2,0]));
    group.add(temp.mesh);
  }
  structures.push({
    id:ss.id||nextStructureId++,group,parts:[...parts],totalMass:ss.totalMass||1,
    hardness:ss.hardness||0,fertility:ss.fertility||0,length:ss.length||0,age:ss.age||0
  });
}
...
function restoreWorld(s){
  ...
  for(const m of s.materials||[]){const mm=makeMaterial(m.type,arrToVec(m.pos));if(mm)mm.staticTime=m.staticTime||0;}
