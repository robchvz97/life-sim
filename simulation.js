  ...
  for(const m of materials){
    ...
    if(inPond(pos)){
      ...
      if(capacity>.065){
        ...
        m.waterAmount=clamp(before+dt*.075*capacity*openness,0,capacity);
        m.waterTemp+=(16-m.waterTemp)*clamp(dt*.28,0,.35);
        if(before<.02&&m.waterAmount>.025)recordDiscovery('container:water:first','💧','Primer objeto con agua','Un objeto con cavidad retuvo agua simplemente por inmersión.',pos);
      }
...
function maybeTransferResources(a,p,dt){
  ...
  if(a.energy>68&&b.energy<58&&Math.random()<1-Math.exp(-dt*.040*Math.max(0,willingness+.05))){
    const amount=Math.min(7,a.energy-62,62-b.energy);
    if(amount>1){a.energy-=amount;b.energy+=amount*.92;recordTransfer(a,b,amount,'food');}
  }
  if(a.carrying&&!b.carrying&&Math.random()<1-Math.exp(-dt*.018*Math.max(0,willingness))){
    const m=a.carrying;
    dropMaterial(a,true);
  ...
  for(const a of agents){
    ...
    while(q.length){
      ...
      for(const b of agents){
        ...
        const affinity=x.socialMemory.get(b.id)?.affinity||0;
        const linked=kinshipScore(x,b)>.15||reputationScore(x,b)>.06||affinity>.12;
        if(x.mesh.position.distanceTo(b.mesh.position)<4.6&&linked){seen.add(b.id);q.push(b);}
      }
    }
    if(count>=3)groups++;
  }return groups;
...
function technicalSense(a,p){
  ...
  return {
    ...
    resistance,
    inWater:inPond(a.mesh.position)?1:0
  };
}
...
function normalizeProcedureRecord(r){
  if(!r)return null;
  if(Array.isArray(r.steps))return {...r,steps:r.steps.map(x=>({s:x.s||'legacy',a:+x.a||0,o:x.o||x.s||'legacy'}))};
  if(Array.isArray(r.seq)){
    const steps=r.seq.map(x=>typeof x==='number'?{s:'legacy',a:x,o:'legacy'}:{s:x.s||'legacy',a:+(x.a??x.mode??0),o:x.o||x.s||'legacy'});
    return {...r,steps};
  }
  return {...r,steps:[]};
}
...
function updateProcedureMemory(a,p,reward){
  ...
  if(pm.pending){
    ...
    if(pm.history.length>=3){
      ...
      if(!r)r={steps,q:0,n:0,source:a.id,start:steps[0].s,end:steps[2].o,physicalChange:0};
      r.n++;
      const changed=steps.some(x=>x.s!==x.o);
      if(changed)r.physicalChange=(r.physicalChange||0)+1;
      const alpha=clamp(.15/Math.sqrt(r.n),.025,.15);
      r.q+=alpha*(reward-r.q);r.start=steps[0].s;r.end=steps[2].o;
      pm.library.set(key,r);
    }
  }
  pm.pending=null;
  if(pm.library.size>44){
    const keep=[...pm.library.entries()]
      .map(([k,r])=>[k,normalizeProcedureRecord(r)])
      .sort((x,y)=>(y[1].q+y[1].n*.003+(y[1].physicalChange||0)*.002)-(x[1].q+x[1].n*.003+(x[1].physicalChange||0)*.002))
      .slice(0,36);
...
function procedureBias(a,p,mode){
  ...
  for(const raw of pm.library.values()){
    ...
    if(!steps.length||r.q<=0)continue;
    let candidate=0;
    ...
    if(candidate===0&&hist.length>=1&&steps.length>=2){
      ...
      if(transitionKey(h)===transitionKey(steps[0])&&steps[1].s===state&&steps[1].a===mode)candidate=r.q*.72;
    }
    if(candidate===0&&steps[0].s===state&&steps[0].a===mode)candidate=r.q*.38;
...
function imitateProcedure(a,n,dt){
  ...
  const rate=.045*(a.genome.cognition?.imitation||.5)*(a.genome.cognition?.cultureBias||.5);
  if(Math.random()>1-Math.exp(-dt*rate))return;
  const ranked=[...n.procedureMemory.library.entries()]
    .map(([k,r])=>[k,normalizeProcedureRecord(r)])
    .filter(([,r])=>r.steps?.length&&r.q>.012)
    .sort((x,y)=>(y[1].q+(y[1].physicalChange||0)*.01)-(x[1].q+(x[1].physicalChange||0)*.01));
  const best=ranked[0];if(!best)return;
  const [key,r]=best,cur=normalizeProcedureRecord(a.procedureMemory.library.get(key));
  if(!cur||r.q*.55>cur.q){
    a.procedureMemory.library.set(key,{...structuredClone(r),q:r.q*.22,n:.2,source:n.id});
    a.procedureMemory.copied++;totalProcedureCopies++;
    if(totalProcedureCopies===1)recordDiscovery('procedure:first','🧭','Primer procedimiento copiado','Un organismo imitó una secuencia estado→acción→resultado de otro.');
  }
}
function exportProcedures(a){
  return [...a.procedureMemory.library.entries()]
    ...
    .sort((x,y)=>(y[1].q+(y[1].physicalChange||0)*.01)-(x[1].q+(x[1].physicalChange||0)*.01))
    .slice(0,4)
    .map(([k,r])=>[k,{...structuredClone(r),q:r.q*.12,n:.1,source:a.id}]);
}
function importProcedures(a,seed,strength=.7){
  for(const [k,raw] of seed||[]){
    ...
    a.procedureMemory.library.set(k,{...structuredClone(r),q:(r.q||0)*strength,n:(r.n||.1)*strength,source:r.source});
  }
}
...
function procedureCount(){
  return agents.reduce((s,a)=>s+[...a.procedureMemory.library.values()].filter(r=>{r=normalizeProcedureRecord(r);return r?.q>.01&&(r.n||0)>.3;}).length,0);
}
function technicalProcedureCount(){
  return agents.reduce((s,a)=>s+[...a.procedureMemory.library.values()].filter(r=>{r=normalizeProcedureRecord(r);return r?.q>.01&&(r.physicalChange||0)>0;}).length,0);
}
...
function chooseDecision(a,per,out){
  ...
  for(const c of candidates){
    ...
    if(per.nVisible&&per.n){const kin=kinshipScore(a,per.n),rep=reputationScore(a,per.n);if(c.mode===2)score+=(kin*(a.genome.cognition?.kinBias||.4)+Math.max(0,rep)*(a.genome.cognition?.reciprocityBias||.5)+(per.mem?.affinity||0)*.45)*.09;if(c.mode===7)score+=(Math.max(0,-rep)*(a.genome.cognition?.territoriality||.3)-kin*.08)*.05;}
...
function updateAgent(a,dt){
  ...
  if(manipIntent>.28){
    if(!a.carrying&&p.m&&p.md<manipRadius){
      ...
      const pushStrength=g.grip*toolBonus*rolling/(effectiveMaterialMass(p.m)+.2);
...
function snapshotAgent(a){
  return {
    ...
    carryingData:a.carrying?{type:a.carrying.type,props:a.carrying.props,waterAmount:a.carrying.waterAmount,waterTemp:a.carrying.waterTemp,softness:a.carrying.softness,shaping:a.carrying.shaping,rotationTurns:a.carrying.rotationTurns,mechanicalWork:a.carrying.mechanicalWork,temperature:a.carrying.temperature,wetness:a.carrying.wetness,thermalState:a.carrying.thermalState,transformedLevel:a.carrying.transformedLevel,originRegion:a.carrying.originRegion,lastRegion:a.carrying.lastRegion,regionalMoves:a.carrying.regionalMoves}:null
  };
}
