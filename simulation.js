...
function maybeLeaveSurfaceTrace(a,p,manipIntent,dt,speed){
  ...
  const threshold=.34+(.28/(.35+ability));
...
function updateAgent(a,dt){
  ...
  if(isFertile(a) && agents.length+embryos.length<CONFIG.MAX_AGENTS+CONFIG.EMBRYO_MAX){
    ...
    if(mate){
      ...
      const lowBoost=agents.length<12?3.2:agents.length<30?2.05:agents.length<60?1.28:1;
      const fertilityScarcity=fertileNow<4?1.32:fertileNow<8?1.14:1;
      const compatibility=clamp(mateCompatibility(a,mate)+.20,.15,1.35);
      const fertilityRate=.038*(.22+.39*ea+.39*eb)*ecology*lowBoost*fertilityScarcity*compatibility;
