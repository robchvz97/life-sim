...
const CONFIG={
  ...
  BASE_METABOLISM:.178, MOVE_COST:.095, WATER_DRAIN:.12,
...
let totalShapingEvents=0,totalProcedureCopies=0,totalFluidTransfers=0,totalMechanicalWork=0,totalWaterContacts=0;
  ...
  for(const m of materials){
    ...
    if(inPond(pos)){
      ...
      if(capacity>.065){
        ...
        m.waterAmount=clamp(before+dt*.62*capacity*openness,0,capacity);
...
function snapshotWorld(){
  return {
    ...
    totalShapingEvents,totalProcedureCopies,totalFluidTransfers,totalMechanicalWork,totalWaterContacts,
...
function restoreWorld(s){
  ...
  totalFluidTransfers=s.totalFluidTransfers||0;totalMechanicalWork=s.totalMechanicalWork||0;totalWaterContacts=s.totalWaterContacts||0;
...
function seed(){
  ...
  totalShapingEvents=0;totalProcedureCopies=0;totalFluidTransfers=0;totalMechanicalWork=0;totalWaterContacts=0;
  while(worldMarks.length){const wm=worldMarks.pop();markGroup.remove(wm.group);wm.group?.traverse(o=>{if(o.geometry)o.geometry.dispose();if(o.material)o.material.dispose();});}
