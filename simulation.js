// Life Sim v17.7.3 bootstrap. The full v17.7.2 engine is pinned to the previous commit,
// transformed in memory, and executed as v17.7.3. This keeps the deployed patch small and reversible.
...
async function loadLifeSim1773(){
 const response=await fetch('https://cdn.jsdelivr.net/gh/robchvz97/life-sim@ae8d6342646db6090ce4e745cbac4086aefdea1c/simulation.js',{cache:'no-store'});
 if(!response.ok)throw new Error(`No se pudo leer simulation.js (${response.status})`);
 let source=await response.text();
 if(source.length!==EXPECTED_LENGTH)throw new Error(`simulation.js inesperado: longitud ${source.length}, esperaba ${EXPECTED_LENGTH}`);
 if(await sha256(source)!==EXPECTED_SHA256)throw new Error('simulation.js no coincide con la base v17.7.2 esperada');
 for(const op of [...OPS].sort((a,b)=>b.start-a.start))source=source.slice(0,op.start)+op.text+source.slice(op.end);
 const blob=new Blob([source],{type:'text/javascript'});const url=URL.createObjectURL(blob);
