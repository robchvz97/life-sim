...
const CONFIG={
  ...
  BIOSPHERE_LOW_POP:24, BIOSPHERE_SEED_TARGET:18,
  ...
  for(const m of materials){
    ...
    if(rain>.03 && !m.carriedBy){
      ...
      if(capRain>.065){
        ...
        m.waterAmount=clamp(beforeRain+dt*.095*rain*capRain*openness,0,capRain);
        m.waterTemp+=(18-m.waterTemp)*clamp(dt*.12*rain,0,.22);
        if(beforeRain<.006&&m.waterAmount>.008){
          totalRainCaptures++;
...
function applyFluidAndMechanics(a,p,manipIntent,dt){
  ...
  if(rain>.10){
    ...
    held.waterAmount=clamp(beforeRain+dt*.055*rain*cap*openness,0,cap);
    held.waterTemp+=(18-held.waterTemp)*clamp(dt*.10*rain,0,.18);
    if(beforeRain<.006&&held.waterAmount>.008){
      totalRainCaptures++;
...
function stabilizeLoadedPopulation(){
  ...
  if(agents.length<CONFIG.BIOSPHERE_LOW_POP || (agents.length<CONFIG.CULTURE_MIN_POP&&noFertile&&averageAge()>45)){
...
function maintainBiosphereContinuity(dt){
  ...
  if(agents.length<CONFIG.BIOSPHERE_LOW_POP || (agents.length<CONFIG.CULTURE_MIN_POP&&fertile===0&&averageAge()>48)){
    const urgency=agents.length<12?.26:agents.length<18?.16:.10;
