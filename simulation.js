...
function randomGenome(){
  ...
  return {
    ...
    brain:randomBrain(44,hiddenSize,18)
  };
}
...
function mutateGenome(g){
  ...
  resizeBrain(ng.brain,c.hiddenSize,44,18);
...
function updateAgent(a,dt){
  ...
  const brainCost=brainNeurons*.00115 + cognition.planningDepth*.0045;
  a.energy-=dt*(CONFIG.BASE_METABOLISM*(.8+g.size*.35+g.segments*.035)+moveCost+carryCost+brainCost+(inPond(a.mesh.position)?CONFIG.WATER_DRAIN:0))*shelter;
...
function seed(){
  ...
  births=0;deaths=0;worldAge=0;totalMentalSims=0;totalImitations=0;nextId=1;nextMaterialId=1;nextStructureId=1;selectedAgent=null;
