# Life Sim v5.1 — Cognitive Update
...
Esta revisión corrige un problema de despliegue/visualización observado en GitHub Pages:

- `simulation.js` ahora lleva un parámetro de versión (`?v=5.1.2`) para evitar que el navegador conserve una copia antigua en caché.
- La cámara se orienta explícitamente al centro del mundo.
- Se fuerza el tamaño/posición del canvas WebGL.
- Se añadió una cuadrícula tenue y un marcador central para confirmar visualmente que el motor 3D está renderizando.
- El panel muestra `Motor cargado` y `Render 3D`. Si aparece `v5.1.2` y `activo · N seres`, el archivo JavaScript correcto está ejecutándose.
- Con esta versión, `Celdas exploradas` debe comenzar por encima de cero inmediatamente, porque cada criatura registra su celda inicial al nacer.
